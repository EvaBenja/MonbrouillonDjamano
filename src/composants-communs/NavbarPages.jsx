...
import ModalChoixEvenement from './ModalChoixEvenement';
...
const NavbarPages = () => {
  ...
  return (
    <>
      <nav style={{ position:'sticky', top:0, zIndex:1000, background:'rgba(255,255,255,0.97)', backdropFilter:'blur(12px)', borderBottom:'1px solid #f0f0f0', fontFamily:'Poppins,sans-serif' }}>
          ...
          {!isMobile && (
            <div style={{ display:'flex', alignItems:'center', gap:4 }}>
              {liens.map(({ label, Icon, to }) => {
                ...
                if (label === 'Évènements') return (
                  <div key={label} onClick={() => setModalEvenement(true)} style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 15px', borderRadius:9, fontSize:13.5, fontWeight:active?700:500, color:active?'#FF5A00':'#444', borderBottom:active?'2.5px solid #FF5A00':'2.5px solid transparent', transition:'all .18s', fontFamily:'Poppins,sans-serif', cursor:'pointer' }}>
                    <Icon /> {label}
                  </div>
                );
                return (
        ...
        {isMobile && menuOpen && (
          <div style={{ background:'#fff', borderTop:'1px solid #f0f0f0', padding:'8px 0 16px' }}>
            ...
            <div onClick={() => { setMenuOpen(false); setModalEvenement(true); }} style={{ display:'flex', alignItems:'center', gap:12, padding:'13px 24px', fontSize:15, fontWeight:400, color:'#333', fontFamily:'Poppins,sans-serif', cursor:'pointer' }}>
              Évènements
            </div>
            {[
      ...
      {/* ── Modal choix Visiteur / Acteur Culturel ── */}
