import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import PiedDePage from '../components/PiedDePage';

/* ── Images depuis /public/experiences/ ── */
const imgs = [
  '/experiences/personne1.jpg','/experiences/personne2.jpg','/experiences/personne3.jpg',
  '/experiences/personne4.jpg','/experiences/personne5.jpg','/experiences/personne6.jpg',
  '/experiences/personne7.jpg','/experiences/personne8.jpg','/experiences/personne9.jpg',
  '/experiences/personne10.jpg',
];
const fallbacks = [
  'linear-gradient(145deg,#1d4ed8,#4338ca)',
  'linear-gradient(145deg,#dc2626,#b91c1c)',
  'linear-gradient(145deg,#0891b2,#0369a1)',
  'linear-gradient(145deg,#065f46,#047857)',
  'linear-gradient(145deg,#7c3aed,#6d28d9)',
  'linear-gradient(145deg,#b45309,#92400e)',
  'linear-gradient(145deg,#be185d,#9d174d)',
  'linear-gradient(145deg,#0f766e,#0d9488)',
  'linear-gradient(145deg,#1e40af,#1d4ed8)',
  'linear-gradient(145deg,#4338ca,#6366f1)',
];

const SECTIONS = [
  {
    id: 'ouaga',
    titre: "Expériences d'évènements · Ouagadougou",
    items: [
      { id:1, nom:'Jean Emmanuel Bitié', role:'Utilisateur',         hybride:false, certifie:false, views:'5.4k',  temps:"il y'a 12h",        texte:"Sortie de notre promotion, comme le temps fil aussi vite qu'on l'i ...", img:0 },
      { id:2, nom:'Marienne Barry',      role:'Utilisateur Hybride', hybride:true,  certifie:false, views:'10.8k', temps:"il y'a 22h",        texte:"Sortie de notre promotion, comme le temps fil aussi vite qu'on l'i ...", img:1 },
      { id:3, nom:'Edouard Ouédraogo',   role:'Utilisateur',         hybride:false, certifie:false, views:'5.4k',  temps:"il y'a 1 Jour",     texte:"Sortie de notre promotion, comme le temps fil aussi vite qu'on l'i ...", img:2 },
      { id:9, nom:'Yasmine Traoré',      role:'Utilisateur',         hybride:false, certifie:false, views:'8k',    temps:'10 Octobre',        texte:"Sortie de notre promotion, comme le temps fil aussi vite qu'on l'i ...", img:3 },
    ],
  },
  {
    id: 'bobo',
    titre: "Expériences d'évènements · Bobo Dioulasso",
    items: [
      { id:4, nom:'Maïmouna Traoré',     role:'Utilisateur Hybride', hybride:true,  certifie:false, views:'50k',  temps:"il y'a 24h",         texte:"Sortie de notre promotion, comme le temps fil aussi vite qu'on l'i ...", img:4 },
      { id:5, nom:'Jean Emmanuel Bitié', role:'Utilisateur',         hybride:false, certifie:false, views:'6.4k', temps:"il y'a 3 jours",     texte:"Sortie de notre promotion, comme le temps fil aussi vite qu'on l'i ...", img:5 },
      { id:6, nom:'Jean Emmanuel Bitié', role:'Utilisateur',         hybride:false, certifie:false, views:'9.9k', temps:"il y'a une semaine", texte:"Sortie de notre promotion, comme le temps fil aussi vite qu'on l'i ...", img:6 },
    ],
  },
  {
    id: 'ailleurs',
    titre: "Expériences d'évènements · D'ailleurs",
    items: [
      { id:7,  nom:'Donald Dao',          role:'Utilisateur Hybride', hybride:true,  certifie:false, views:'15k', temps:'Hier',         texte:"Sortie de notre promotion, comme le temps fil aussi vite qu'on l'i ...", img:7 },
      { id:8,  nom:'Yasmine Traoré',      role:'Utilisateur',         hybride:false, certifie:false, views:'8k',  temps:'10 Octobre',   texte:"Sortie de notre promotion, comme le temps fil aussi vite qu'on l'i ...", img:8 },
      { id:10, nom:'Darlène Jad Nikiema', role:'Utilisateur',         hybride:false, certifie:false, views:'25k', temps:'22 décembre',  texte:"Sortie de notre promotion, comme le temps fil aussi vite qu'on l'i ...", img:9 },
    ],
  },
];

/* ─── Icônes ─── */
const IcoLeft  = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="15 18 9 12 15 6"/></svg>;
const IcoRight = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg>;

/* Icône partager — trait plus épais pour "gras" */
const IcoShare = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#555" strokeWidth="2.2" strokeLinecap="round">
    <circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/>
    <line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/>
    <line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/>
  </svg>
);

/* ─── Carte Expérience ─── */
const CarteExperience = ({ exp, onClick }) => {
  const [imgError, setImgError] = useState(false);
  const imgSrc  = imgs[exp.img] || imgs[0];
  const fallback = fallbacks[exp.img % fallbacks.length];

  return (
    <div
      onClick={onClick}
      style={{
        background: '#fff', borderRadius: 14, overflow: 'hidden',
        border: '1px solid #efefef', cursor: 'pointer',
        boxShadow: '0 1px 6px rgba(0,0,0,.05)',
        fontFamily: 'Poppins, sans-serif',
      }}
    >
      {/* ── En-tête user ── */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '11px 13px 8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
          {/* Avatar initiale */}
          <div style={{
            width: 36, height: 36, borderRadius: '50%',
            background: fallback,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: '#fff', fontSize: 13, fontWeight: 700, flexShrink: 0,
          }}>
            {exp.nom[0]}
          </div>
          <div>
            {/* Nom */}
            <div style={{ fontSize: 12.5, fontWeight: 700, color: '#111' }}>{exp.nom}</div>
            {/* Rôle + badge hybride rouge si applicable */}
            <div style={{ fontSize: 11, color: '#aaa', display: 'flex', alignItems: 'center', gap: 4 }}>
              {exp.hybride && (
                <span style={{ color: '#ef4444', fontSize: 10, fontWeight: 700 }}>●</span>
              )}
              {exp.certifie && (
                <span style={{ color: '#ef4444', fontSize: 10, fontWeight: 700 }}>✓</span>
              )}
              {exp.role}
            </div>
          </div>
        </div>
        {/* Menu 3 points */}
        <span style={{ fontSize: 18, color: '#ccc', letterSpacing: '1px', cursor: 'pointer' }}>⋮</span>
      </div>

      {/* ── Photo ── */}
      <div style={{
        width: '100%', height: 190,
        background: imgError ? fallback : '#f5f5f5',
        overflow: 'hidden',
      }}>
        {!imgError ? (
          <img
            src={imgSrc}
            alt={exp.nom}
            onError={() => setImgError(true)}
            style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
          />
        ) : (
          <div style={{ width: '100%', height: '100%', background: fallback }} />
        )}
      </div>

      {/* ── Corps ── */}
      <div style={{ padding: '10px 13px 13px' }}>
        {/* Vues + temps en GRAS */}
        <div style={{ marginBottom: 5 }}>
          <span style={{ fontSize: 12.5, fontWeight: 700, color: '#111' }}>{exp.views},</span>
          <span style={{ fontSize: 12.5, fontWeight: 700, color: '#111', marginLeft: 4 }}>{exp.temps}</span>
        </div>
        {/* Texte */}
        <div style={{ fontSize: 12, color: '#666', lineHeight: 1.65, marginBottom: 8 }}>
          {exp.texte}
        </div>
        {/* Footer : Afficher plus en NOIR gras + share */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{
            fontSize: 12, fontWeight: 700, color: '#111', cursor: 'pointer',
          }}>
            Afficher plus ...
          </span>
          <IcoShare />
        </div>
      </div>
    </div>
  );
};

/* ─── Section avec slider ─── */
const SectionSlider = ({ section, onSelect }) => {
  const [start, setStart] = useState(0);
  const PER = 3;
  const canLeft  = start > 0;
  const canRight = start + PER < section.items.length;

  const nav = (active, side) => ({
    position: 'absolute', top: '50%', transform: 'translateY(-50%)',
    [side]: -18, width: 36, height: 36, borderRadius: '50%',
    background: active ? '#fff' : 'rgba(255,255,255,0.4)',
    boxShadow: active ? '0 2px 14px rgba(0,0,0,.14)' : 'none',
    border: '1.5px solid #eee',
    display: 'flex', alignItems: 'center', justifyContent: 'center',
    cursor: active ? 'pointer' : 'default',
    opacity: active ? 1 : 0.3, zIndex: 2, color: '#333',
  });

  return (
    <div style={{ marginBottom: 52 }}>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 20 }}>
        <span style={{ fontSize: 16, fontWeight: 700, color: '#111', fontFamily: 'Poppins,sans-serif', flex: 1 }}>
          {section.titre}
        </span>
        <span style={{ fontSize: 13, color: '#FF5A00', fontWeight: 700 }}>&nbsp;›</span>
      </div>
      <div style={{ position: 'relative' }}>
        <div style={nav(canLeft, 'left')}  onClick={() => canLeft  && setStart(s => s - 1)}><IcoLeft /></div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 22 }}>
          {section.items.slice(start, start + PER).map(exp => (
            <CarteExperience key={exp.id} exp={exp} onClick={() => onSelect(exp.id)} />
          ))}
        </div>
        <div style={nav(canRight, 'right')} onClick={() => canRight && setStart(s => s + 1)}><IcoRight /></div>
      </div>
    </div>
  );
};

/* ─── Barre recherche ─── */
const BarreRecherche = () => (
  <div style={{
    background: '#fff', borderRadius: 16,
    boxShadow: '0 4px 24px rgba(0,0,0,.09)',
    border: '1px solid #efefef',
    display: 'flex', alignItems: 'stretch', overflow: 'hidden',
    fontFamily: 'Poppins,sans-serif',
  }}>
    {['Type de recherche','Localisation','Date'].map((lbl, i) => (
      <div key={lbl} style={{
        flex: 1, padding: '16px 22px',
        borderRight: i < 2 ? '1px solid #efefef' : 'none',
        display: 'flex', flexDirection: 'column', gap: 4,
      }}>
        <span style={{ fontSize: 10, fontWeight: 700, color: '#bbb', letterSpacing: '1px', textTransform: 'uppercase' }}>{lbl}</span>
        <input placeholder={lbl === 'Date' ? 'Quand ?' : 'Ajouter'} style={{ border: 'none', outline: 'none', fontSize: 13.5, color: '#333', background: 'transparent', fontFamily: 'Poppins,sans-serif' }} />
      </div>
    ))}
    <button style={{
      background: '#FF5A00', color: '#fff', border: 'none',
      padding: '0 28px', fontSize: 14, fontWeight: 600, fontFamily: 'Poppins,sans-serif', cursor: 'pointer',
      display: 'flex', alignItems: 'center', gap: 8,
    }}>
      <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
      Rechercher
    </button>
  </div>
);

/* ─── Pagination ─── */
const Pagination = ({ page, total, onChange }) => (
  <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, padding: '20px 0 44px', fontFamily: 'Poppins,sans-serif' }}>
    <div onClick={() => page > 1 && onChange(page - 1)} style={{ width:34,height:34,borderRadius:'50%',border:'1.5px solid #e0e0e0',background:'#fff',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:'#555' }}><IcoLeft /></div>
    {[...Array(total)].map((_,i) => (
      <div key={i} onClick={() => onChange(i+1)} style={{ width:36,height:36,borderRadius:'50%',border:i+1===page?'none':'1.5px solid #e0e0e0',background:i+1===page?'#FF5A00':'#fff',color:i+1===page?'#fff':'#555',fontSize:13,fontWeight:i+1===page?700:400,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center' }}>{i+1}</div>
    ))}
    <div onClick={() => page < total && onChange(page + 1)} style={{ width:34,height:34,borderRadius:'50%',border:'1.5px solid #e0e0e0',background:'#fff',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',color:'#555' }}><IcoRight /></div>
  </div>
);

/* ══ PAGE EXPÉRIENCES ══ */
const PageExperiences = () => {
  const navigate = useNavigate();
  const [page, setPage] = useState(1);
  return (
    <div style={{ background: '#fff', minHeight: '100vh', fontFamily: 'Poppins, sans-serif' }}>
      <div style={{ maxWidth: 1160, margin: '0 auto', padding: '28px 28px 0' }}>
        <BarreRecherche />
      </div>
      <div style={{ maxWidth: 1160, margin: '0 auto', padding: '48px 28px 0' }}>
        {SECTIONS.map(sec => (
          <SectionSlider key={sec.id} section={sec} onSelect={id => navigate(`/experiences/${id}`)} />
        ))}
        <Pagination page={page} total={4} onChange={setPage} />
      </div>
      <PiedDePage />
    </div>
  );
};

export default PageExperiences;
