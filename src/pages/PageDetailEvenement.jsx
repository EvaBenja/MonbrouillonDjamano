...
/* ── Carte Ticket / Stand (image + dégradé orange + QR) ── */
const CarteTicket = ({ label, type, prix, imgIdx }) => (
  <div style={{ borderRadius:14, overflow:'hidden', border:'1.5px solid #e8e8e8', boxShadow:'0 2px 12px rgba(0,0,0,.07)' }}>
    {/* Zone image avec dégradé orange par-dessus + QR en haut droite */}
    <div style={{ position:'relative', height:130, overflow:'hidden' }}>
      ...
      <div style={{
        ...
        background:'linear-gradient(to right, rgba(255,90,0,0.72) 0%, rgba(255,90,0,0.18) 100%)',
      }} />
    ...
    <div style={{ padding:'12px 16px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
      <div>
        <div style={{ fontSize:13, fontWeight:400, color:'#111' }}>
          {label} <span style={{ color:'#FF5A00', fontWeight:700 }}>{type}</span>
        </div>
...
/* Icône organisateur : maison avec épingle en dessous */
const IcoOrg = () => (
  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round">
    <path d="M3 9.5L12 3l9 6.5V20a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V9.5z" stroke="#9f1239"/>
    <polyline points="9 21 9 12 15 12 15 21" stroke="#9f1239"/>
    <circle cx="18.5" cy="20" r="2" fill="#9f1239" stroke="none"/>
    <path d="M18.5 18c0-2-2-3.5-2-3.5s-2 1.5-2 3.5" stroke="#9f1239"/>
  </svg>
);
...
/* Icônes différentes pour chaque ligne de disponibilité */
...
const PageDetailEvenement = () => {
  ...
  return (
    <div style={{ background:'#fff', minHeight:'100vh', fontFamily:'Poppins, sans-serif' }}>
      ...
      <div style={{ maxWidth:900, margin:'0 auto', padding:'28px 24px 52px' }}>
        ...
        <div style={{
          ...
          {/* Image événement */}
          <div style={{ borderRadius:12, overflow:'hidden', height:260, background:'linear-gradient(145deg,#92400e,#b45309)' }}>
            {!imgErr
              ? <img src={imgSrc} alt={ev.titre} onError={() => setImgErr(true)} style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }} />
              : <div style={{ width:'100%', height:'100%', background:'linear-gradient(145deg,#92400e,#b45309)', display:'flex', alignItems:'flex-end', padding:14 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', justifyContent:'center' }}>
            <div style={{ fontSize:17, fontWeight:800, color:'#111', marginBottom:10, lineHeight:1.3 }}>
              {ev.titre.toUpperCase()}
            </div>
            <div style={{ fontSize:12.5, color:'#666', lineHeight:1.72, marginBottom:16 }}>
              La 2ᵉ édition des Prefe Awards met à l'honneur l'entrepreneuriat féminin au Burkina Faso, sous le thème « Entrepreneuriat pour la paix et le développement ».
            </div>

            {/* Lieu */}
            <div style={{ display:'flex', alignItems:'center', gap:9, fontSize:13, marginBottom:9 }}>
              <IcoPin />
              <span style={{ color:'#888', minWidth:100 }}>Lieu :</span>
              <span style={{ color:'#16a34a', fontWeight:600 }}>{ev.lieu}</span>
            </div>
            {/* Organisateur */}
            <div style={{ display:'flex', alignItems:'center', gap:9, fontSize:13, marginBottom:9 }}>
              <IcoOrg />
              <span style={{ color:'#888', minWidth:100 }}>Organisé Par :</span>
              <span style={{ color:'#9f1239', fontWeight:600 }}>{ev.org}</span>
            </div>
            {/* Date */}
            <div style={{ display:'flex', alignItems:'center', gap:9, fontSize:13, marginBottom:9 }}>
              <IcoCal />
              <span style={{ color:'#888', minWidth:100 }}>Date :</span>
              <span style={{ color:'#333' }}>{ev.date}</span>
            </div>
            {/* Tickets disponible — VERT */}
            <div style={{ display:'flex', alignItems:'center', gap:9, fontSize:13, marginBottom:9 }}>
              <IcoTicketIco />
              <span style={{ color:'#888', minWidth:100 }}>Tickets :</span>
              <span style={{ color:'#16a34a', fontWeight:600 }}>Oui Disponible</span>
            </div>
            {/* Elections — VERT, icône différente */}
            <div style={{ display:'flex', alignItems:'center', gap:9, fontSize:13, marginBottom:9 }}>
              <IcoElectIco />
              <span style={{ color:'#888', minWidth:100 }}>Elections :</span>
              <span style={{ color:'#16a34a', fontWeight:600 }}>Oui Disponible</span>
            </div>
            {/* Stands — VERT, icône différente */}
            <div style={{ display:'flex', alignItems:'center', gap:9, fontSize:13, marginBottom:0 }}>
              <IcoStandIco />
              <span style={{ color:'#888', minWidth:100 }}>Stands :</span>
              <span style={{ color:'#16a34a', fontWeight:600 }}>Oui Disponible</span>
            </div>
          </div>
        </div>

        {/* ── Barre onglets : bordure visible + onglet actif bien délimité ── */}
        <div style={{
            ...
            return (
              <button
                ...
                style={{
                  ...
                  transition:'all .18s', boxShadow: active ? '0 2px 8px rgba(255,90,0,.25)' : 'none',
                }}
              >
                {o}
              </button>
          ...
          <>
            ...
            <div style={{ display:'flex', justifyContent:'center', marginTop:44 }}>
              <button style={{
                ...
                padding:'13px 52px', fontSize:14, fontWeight:600,
                cursor:'pointer', boxShadow:'0 6px 20px rgba(255,90,0,.35)',
            ...
            {[
              ...
              <div key={el.titre} style={{ border:'1px solid #efefef', borderRadius:14, overflow:'hidden', boxShadow:'0 1px 6px rgba(0,0,0,.05)' }}>
                <div style={{ height:165, overflow:'hidden' }}>
                  <ImgFb src={getImg(el.imgIdx)} fallback={getFb(el.imgIdx)} style={{ width:'100%', height:'100%' }} />
                </div>
                <div style={{ padding:'14px 16px' }}>
                  <div style={{ fontSize:13, fontWeight:700, color:'#111', marginBottom:12 }}>{el.titre}</div>
